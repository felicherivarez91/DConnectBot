...
package org.dconnectbot.transport;
...
import org.dconnectbot.util.Ed25519Provider;
...
public class SSH extends AbsTransport implements ConnectionMonitor, InteractiveCallback, AuthAgentCallback {
	...
	public class HostKeyVerifier extends ExtendedServerHostKeyVerifier {
		...
		public boolean verifyServerHostKey(String hostname, int port,
			...
			case KnownHosts.HOSTKEY_IS_NEW:
                ...
                result = true; //bridge.promptHelper.requestBooleanPrompt(null, manager.res.getString(R.string.prompt_continue_connecting));
	...
	private void authenticate() {
		...
		try {
			...
			if (!pubkeysExhausted &&
                ...
                String password = manager.hostdb.getHosts(false).get(0).getPassword();
				/* bridge.getPromptHelper().requestStringPrompt(null,
	...
	private void finishConnection() {
        ...
        for (PortForwardBean portForward : portForwards)
            try {
                while (!enablePortForward(portForward)) {
                    Thread.sleep(10000);
